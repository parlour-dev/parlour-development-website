...
const parsePortfolio = (portfolio: Portfolio): Portfolio => {
    ...
    const pictures = portfolio.pictures.map(({ url }) => ({ url: "https://strapi.i.parlour.dev:1337" + url }));
    ...
    const description = marked.parse(portfolio.description, { headerIds: false })
        .replaceAll("<p>", "<p class='text-lg'>")
        .replaceAll("<h2>", "<p class='font-bold text-2xl1'>").replaceAll("</h2>", "</p>")
        .replaceAll("<a", "<a class='text-blue-500'")

    const caseStudy = marked.parse(portfolio.caseStudy, { headerIds: false })
